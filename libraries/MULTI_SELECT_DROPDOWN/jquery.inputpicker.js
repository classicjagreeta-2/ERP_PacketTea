...
(function (factory) {
        ...
        show: function (e) {
            return this.each(function () {
                ...
                if (!_isInputVisible(input)) {
                    ...
                    dd('_render');
                    _set(input, 'pageCurrent', 1);   // start fresh at page 1 on open
    ...
    function _initPP() {
        if ($.fn.popover) { $('[data-toggle="popover"]').popover(); }   // no-op if Bootstrap isn't loaded
    }
    ...
    function _getWrappedList(input) {
        ...
        if (!wrapped_list.length) {
            ...
            if (wrapped_list.popover) { wrapped_list.popover(); }   // no-op if Bootstrap isn't loaded
    ...
    /**
     * Slice the filtered data into the current page.
     * Used by multi-select/checkbox rendering so that "All" only ever
     * acts on the rows that are actually on the current page.
     * @param input
     * @private
     */
    function _getPageData(input) {
        var allData = _filterData(input);
        var paginated = !!(_pagination(input) && !_set(input, 'url'));
        var limit = parseInt(_set(input, 'limit')) || 10;
        var current = parseInt(_set(input, 'pageCurrent')) || 1;
        var totalPages = 1;
        var pageData = allData;

        if (paginated) {
            totalPages = Math.max(1, Math.ceil(allData.length / limit));
            if (current > totalPages) current = totalPages;
            if (current < 1) current = 1;
            _set(input, 'pageCurrent', current);
            pageData = allData.slice((current - 1) * limit, current * limit);
        }
    ...
    function _renderTableHeader(input, hasCheckbox) {
        var output = "";
        var fields = _set(input, 'fields');
        if (_set(input, 'headShow')) {
            output += '<thead><tr>';
            if (hasCheckbox) {
                output += '<th class="inputpicker-checkbox-col"></th>';
            }
    ...
    function _dataRenderMultiple(input) {
        ...
        var uuid = _uuid(input);
        // var data = _set(input, 'data');
        var pageInfo = _getPageData(input);
        var data = pageInfo.pageData;   // current page only - also scopes "All"
        var fields = _set(input, 'fields');
        var fieldValue = _set(input, 'fieldValue');
        ...
        var showCheckbox = _set(input, 'checkbox') !== false;
        var showSelectAll = showCheckbox && _set(input, 'selectAll') !== false;
        var selectAllText = _set(input, 'selectAllText') || 'All';
        ...
        // Draw table
        output += "<table class=\"table" + (showCheckbox ? ' inputpicker-table-checkbox' : '') + "\">";

        // Show head
        output += _renderTableHeader(input, showCheckbox);

        // Show data
        if (data.length) {
            output += "<tbody>";

            // "All" row - selects/deselects every row that matches the current filter,
            // across every page (not just the page currently in view)
            if (showSelectAll) {
                ...
                output += '<tr class="inputpicker-select-all' + (allSelected ? ' inputpicker-selected' : '') + '">';
                output += '<td class="inputpicker-checkbox-col"><input type="checkbox" class="inputpicker-checkbox" tabindex="-1" ' + (allSelected ? 'checked="checked"' : '') + ' /></td>';
                output += '<td colspan="' + Math.max(fields.length, 1) + '"><b>' + selectAllText + '</b></td>';
                output += '</tr>';
            }
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (showCheckbox) {
                    output += '<td class="inputpicker-checkbox-col"><input type="checkbox" class="inputpicker-checkbox" tabindex="-1" ' + (isSelected ? 'checked="checked"' : '') + ' /></td>';
                }
            ...
            output += "<thead><tr><td colspan='" + (fields.length + (showCheckbox ? 1 : 0)) + "' align='center' class=\"inputpicker-no-result\">No results.</td></thead></tr>";
        }

        // Page footer - only the current page's rows are ever selected by "All"
        if (pageInfo.paginated && pageInfo.allData.length) {
            var prevPage = pageInfo.current > 1 ? pageInfo.current - 1 : 1;
            var nextPage = pageInfo.current < pageInfo.totalPages ? pageInfo.current + 1 : pageInfo.totalPages;
            var colspan = fields.length + (showCheckbox ? 1 : 0);

            output += '<tfoot class="inputpicker-pagination"><tr><td colspan="' + colspan + '">';
            output += '<div style="width:100%;">';
            output += '<div style="float:left;padding-left:5px;">' + pageInfo.allData.length + ' result' + (pageInfo.allData.length == 1 ? '' : 's') + '</div>';
            output += '<div style="float:right;padding-right:5px;">' +
                '<a href="javascript:void(0);" onclick="$(\'#inputpicker-' + uuid + '\').inputpicker(\'jumpToPage\', \'1\');">First</a>' +
                '<a href="javascript:void(0);" onclick="$(\'#inputpicker-' + uuid + '\').inputpicker(\'jumpToPage\', \'' + prevPage + '\');">Prev</a>' +
                '<span class="current-page">' + pageInfo.current + ' / ' + pageInfo.totalPages + '</span>' +
                '<a href="javascript:void(0);" onclick="$(\'#inputpicker-' + uuid + '\').inputpicker(\'jumpToPage\', \'' + nextPage + '\');">Next</a>' +
                '<a href="javascript:void(0);" onclick="$(\'#inputpicker-' + uuid + '\').inputpicker(\'jumpToPage\', \'' + pageInfo.totalPages + '\');">Last</a>' +
                '</div>';
        ...
        // Set events - "All" row
        wrapped_list.find('tr.inputpicker-select-all').on('click', function (e) {
            var self = $(this);
            var uuid = $('#inputpicker-wrapped-list').data('inputpicker-uuid');
            var input = $('#inputpicker-' + uuid);
            var delimiter = _set(input, 'delimiter');
            var fieldValue = _set(input, 'fieldValue');
            var currentData = _getPageData(input).allData;   // every filtered row, across all pages
            var wasAllSelected = self.hasClass('inputpicker-selected');
            ...
            if (_set(input, 'selectAllClears')) arr_value = [];
                ...
                else {   // Select every row currently shown
                    if (idx == -1) arr_value.push(currentData[k][fieldValue]);
                }
            }

            _setValue(input, arr_value.join(delimiter));

            // Re-render in place so all checkboxes reflect the new state; dropdown stays open.
            _dataRenderMultiple(input);
        ...
        // Set events - data rows
        wrapped_list.find('tbody').find('tr.inputpicker-element').each(function () {
            var that = $(this);
            that.on('mouseover', function (e) {
                ...
                // Re-render in place to refresh row + "All" checkboxes; dropdown stays open
                // so the user can keep picking multiple items.
                _dataRenderMultiple(input);
    ...
    function _eventKeyUp(e) {
        ...
        if (_set(input, 'url')) {
            ...
            if (_set(input, 'filterOpen')) {  // Need to render with filtering
                _set(input, 'pageCurrent', 1);   // new search - back to page 1
                _dataRender(input);
            }
    ...
    $.fn.inputpicker.defaults = {
        ...
        /**
         * Show checkboxes on rows when multiple is true
         */
        checkbox: true,

        /**
         * Show an "All" row (with checkbox) at the top of the list when multiple + checkbox are true
         * Selecting it selects/deselects all currently filtered rows
         */
        selectAll: true,
