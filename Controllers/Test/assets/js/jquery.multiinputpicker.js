...
(function (factory) {
    ...
    function _dataRenderMultiple(input) {
        ...
        if (data.length) {
            ...
            // "All" row - selects/deselects every currently filtered row
            if (showSelectAll) {
                var allSelected = true;
                for (var a = 0; a < data.length; a++) {
                    if (_inArray(data[a][fieldValue], arr_value) == -1) {
                        allSelected = false;
                        break;
                    }
                }
        ...
        wrapped_list.find('tr.inputpicker-select-all').on('click', function (e) {
            ...
            var delimiter = _set(input, 'delimiter');
            var fieldValue = _set(input, 'fieldValue');
            var currentData = _getPageData(input).pageData;   // this page only
